...
export default class MdEdit extends Component {
    ...
    insertImage() {
        ...
        inputObj.onchange = function () {
            ...
            axios({
                ...
                var cm = self.getCodeMirror();
                var format = { before: '![', after: '](' + res.data.url + ')' };
    ...
    toggleFullScreen() {
        var el = this.getCodeMirror().getWrapperElement();
        var doc = document;
            ...
            <div className="MdEditor">
                <div className="MdEditor-toolbar">
                    ...
                    <button type="button" className="MdEditor-button" onClick={() => this.insertLink()} title="链接">
                        <svg className="MdEditor-svg link" fill="currentColor" viewBox="0 0 24 24" width="24" height="24"><path d="M6.77 17.23c-.905-.904-.94-2.333-.08-3.193l3.059-3.06-1.192-1.19-3.059 3.058c-1.489 1.489-1.427 3.954.138 5.519s4.03 1.627 5.519.138l3.059-3.059-1.192-1.192-3.059 3.06c-.86.86-2.289.824-3.193-.08zm3.016-8.673l1.192 1.192 3.059-3.06c.86-.86 2.289-.824 3.193.08.905.905.94 2.334.08 3.194l-3.059 3.06 1.192 1.19 3.059-3.058c1.489-1.489 1.427-3.954-.138-5.519s-4.03-1.627-5.519-.138L9.786 8.557zm-1.023 6.68c.33.33.863.343 1.177.029l5.34-5.34c.314-.314.3-.846-.03-1.176-.33-.33-.862-.344-1.176-.03l-5.34 5.34c-.314.314-.3.846.03 1.177z" fill-rule="evenodd"></path></svg>
                    </button>
                    <button type="button" className="MdEditor-button image" onClick={() => this.insertImage()} title="上传图片" type="button">
                        <svg className="MdEditor-svg" fill="currentColor" viewBox="0 0 24 24" width="24" height="24"><path d="M21 17.444C21 18.3 20.1 19 19 19H5c-1.1 0-2-.7-2-1.556V6.556C3 5.7 3.9 5 5 5h14c1.1 0 2 .7 2 1.556v10.888zm-9.437-3.919a.5.5 0 0 1-.862.013l-1.26-2.065a.5.5 0 0 0-.861.012l-2.153 3.767a.5.5 0 0 0 .435.748h10.292a.5.5 0 0 0 .438-.741L14.573 9.78a.5.5 0 0 0-.872-.006l-2.138 3.75z" fill-rule="evenodd"></path></svg>
                    </button>
                    ...
                    <div style="flex: 1 0 auto;text-align:center"><strong>markdown编辑器，开始写作吧！</strong></div>
                    ...
                    <button type="button" className="MdEditor-button fullscreen" onClick={() => this.toggleFullScreen()} title="全屏">
                        <svg className="MdEditor-svg" viewBox="0 0 512 512" width="20" height="20">
                            <path d="M352.201 425.775l-79.196 79.196c-9.373 9.373-24.568 9.373-33.941 0l-79.196-79.196c-15.119-15.119-4.411-40.971 16.971-40.97h51.162L228 284H127.196v51.162c0 21.382-25.851 32.09-40.971 16.971L7.029 272.937c-9.373-9.373-9.373-24.569 0-33.941L86.225 159.8c15.119-15.119 40.971-4.411 40.971 16.971V228H228V127.196h-51.23c-21.382 0-32.09-25.851-16.971-40.971l79.196-79.196c9.373-9.373 24.568-9.373 33.941 0l79.196 79.196c15.119 15.119 4.411 40.971-16.971 40.971h-51.162V228h100.804v-51.162c0-21.382 25.851-32.09 40.97-16.971l79.196 79.196c9.373 9.373 9.373 24.569 0 33.941L425.773 352.2c-15.119 15.119-40.971 4.411-40.97-16.971V284H284v100.804h51.23c21.382 0 32.09 25.851 16.971 40.971z" class=""></path>
                        </svg>
                    </button>
                </div>
                <div className="MdEditor-wraper">
                    ...
                    {
                        this.state.isPreview && <div class="preview markdown-body" dangerouslySetInnerHTML={{
